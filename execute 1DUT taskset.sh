...
do
    ...
    f1="1DUT_${iteration}_CPU_LIMIT.txt"
    f2="Currpsr.txt"
    
    taskset -a -c 3-4 fio DUT1.txt --eta-newline=1 --eta=always > logs/${f1} & CURR_PID=$! ; echo $CURR_PID ; for i in {1..180}; do echo $(ps -o psr $CURR_PID) >> "Currpsr_$CURR_PID.txt" ; sleep 1; done & sleep 180
